...
public class OrderController {
    ...
    public ResponseEntity<RequestResponse<List<Order>>> getAllUserOrders(HttpServletRequest request){
        Optional<AppUser> user = appUserRepository.findByEmail(String.valueOf(request.getAttribute("username")));
        if(user.isPresent()){
            List<Order> orders = orderRepository.findByUser_Id(user.get().getId());
            return ResponseEntity.ok(new RequestResponse<List<Order>>(true, orders, "User finalized orders"));
        }
        else throw new AppException("User is missing", HttpStatus.NOT_FOUND.value(), "false", true);
    }
    ...
    public ResponseEntity<RequestResponse<Order>> updateOrder(@PathVariable("id") Long id, @Valid @RequestBody JsonNode requestBody, BindingResult bindingResult) {
        ProducerController.CheckValues(bindingResult);
        ...
        if (existingOrder.isPresent()) {
            ...
            // Update the order properties based on the requestBody
            order.setAdditionalInfo(requestBody.get("additional_information").asText());
            order.setStatus(Order.Status.valueOf(requestBody.get("status").asText()));
            orderRepository.save(order);
            RequestResponse<Order> response = new RequestResponse<>(true, order, "Order was updated");
            return ResponseEntity.ok(response);
        }
