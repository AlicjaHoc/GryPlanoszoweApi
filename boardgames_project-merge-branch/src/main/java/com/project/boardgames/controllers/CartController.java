...
public class CartController {
    ...
    @GetMapping("/cart")
    public ResponseEntity<RequestResponse<Cart>> getCart(HttpServletRequest request) {
        ...
        if (cartOptional.isPresent()) {
            ...
            RequestResponse<Cart> response = new RequestResponse<>(true, cart, "User: " + user.getEmail() + " present cart is:");
            return ResponseEntity.ok(response);
        }
        throw new AppException("There is no cart corresponding to logged in user", HttpStatus.NOT_FOUND.value(), "fail", true);
    }
    ...
    @GetMapping("/carts")
    public ResponseEntity<RequestResponse<List<Cart>>> getCarts() {
        List<Cart> carts = cartRepository.findAll();
        if (!carts.isEmpty()) {
            RequestResponse<List<Cart>> response = new RequestResponse<>(true, carts, "All carts:");
            return ResponseEntity.ok(response);
        }
        throw new AppException("There are no carts available", HttpStatus.NOT_FOUND.value(), "fail", true);
    }
