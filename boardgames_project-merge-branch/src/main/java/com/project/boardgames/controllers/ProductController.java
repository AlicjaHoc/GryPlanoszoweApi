    ...
    public ResponseEntity<RequestResponse<Product>> addProduct(
        ...
        try {
            if (!photo.isEmpty()) {
                ...
                String filePath = Paths.get("").toAbsolutePath().toString() + "\\boardgames_project-merge-branch\\src\\main\\resources\\static\\images\\" + newFilename;
                System.out.println("C:\\Users\\alicj\\Downloads\\boardgames_project-merge-branch\\boardgames_project-merge-branch\\src\\main\\resources\\static\\images" == filePath);
                ...
                newProduct.setPhoto(newFilename);
                ...
                response.setMessage(response.getMessage().replace("\\", ""));
                return ResponseEntity.ok(response);
            }
        } catch (IOException e) {
//            throw new AppException("Failed to upload image: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR.value(), "fail", true);
    ...
    public ResponseEntity<RequestResponse<Void>> deleteProduct(@PathVariable("id") Long id) {
        ...
        Optional<Product> productOptional = genericService.returnEntityById(id);
        ...
        // Zero out the quantity of the product in associated carts
        List<CartItem> cartItems = cartItemRepository.findByProduct(product);
        for (CartItem cartItem : cartItems) {
            cartItem.setQuantity(0);
            cartItem.setChangedSinceAdded(true);
